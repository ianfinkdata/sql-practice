# Tier 1 — Beginner

Your first real SQL. This tier covers the core `SELECT` toolkit — filtering, sorting, limiting, NULLs, aggregates, pattern matching, and duplicates — using Oakhaven's calmer tables (`bronze_products`, `bronze_employees`). The messiest data is saved for Tier 2, where cleaning it up is the actual lesson.

| Module | Topic |
|---|---|
| [01](01-select-and-from.md) | SELECT and FROM |
| [02](02-filtering-with-where.md) | Filtering with WHERE |
| [03](03-sorting-with-order-by.md) | Sorting with ORDER BY |
| [04](04-limiting-with-limit.md) | Limiting results with LIMIT |
| [05](05-null-the-absence-of-a-value.md) | NULL: the absence of a value |
| [06](06-basic-aggregate-functions.md) | Basic aggregate functions |
| [07](07-pattern-matching-with-like.md) | Pattern matching with LIKE |
| [08](08-distinct-and-duplicates.md) | DISTINCT and duplicates |

Matching exercises: [`exercises/01-beginner/`](../../exercises/01-beginner/).

Previous: [Tier 0 — Orientation](../00-orientation/README.md). Next: [Tier 2 — Intermediate](../02-intermediate/README.md).
