# Oakhaven Outfitters — DATA CONTRACT v1.3

> **v1.3 (2026-07-13, Ian-approved):** rebuilding on a fresh Ubuntu box after
> the original Windows machine's drive died. `common_db.dim_date` (the
> calendar's source under v1.1/v1.2) does not exist here and has no
> reproducible source of its own, so the `common_db` indirection is dropped:
> `oakhaven.calendar` is now generated directly (recursive CTE,
> `ddl/02_calendar_generate.sql`), carrying only `date_key` + `date` in
> bronze — derived columns (year/month/quarter/week_day/etc.) are deferred to
> the silver layer, not needed for this rebuild. Range widened to
> 2018-01-01–2038-12-31 (7,670 rows, vs. the old 4,748). §3.8 and acceptance
> criterion 10 updated; `ddl/02_calendar_copy.sql` kept in the repo for
> history but is no longer part of the build.
...
### 3.8 calendar — generated directly (v1.3) — 7,670 rows — CLEAN utility dim
Built by `ddl/02_calendar_generate.sql` via a recursive CTE — no external
database dependency, rerunnable, no agent involvement. Bronze shape is
minimal: `date_key` INT 🔒PK (yyyymmdd from `date`) · `date` DATE 🔒 NOT
NULL. Covers 2018-01-01–2038-12-31, deliberately wider than the fact window.
Derived columns (`year`, `month_num`, `month`, `quarter`, `week_day`,
`week_day_name`, `is_weekend`, `week_start`, `iso_week_start`) are NOT in
bronze under v1.3 — they're a silver-layer concern, added when that layer is
actually built. (v1.1/v1.2 sourced this from `common_db.dim_date`, 4,748
rows, 2019–2031; that database doesn't exist on the rebuilt machine and has
no reproducible source, so the indirection was dropped — see the v1.3 note
...
10. **Calendar** (v1.3): 7,670 rows, no gaps (`DATEDIFF(MAX(date),MIN(date))+1
    = COUNT(*)`), MIN(date) ≤ 2019-01-01 and MAX(date) ≥ 2026-06-30, full span
    2018-01-01–2038-12-31. No source-database equality check (dropped along
    with `common_db` — see v1.3 note).
